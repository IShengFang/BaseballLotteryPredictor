...
def get_1(year, data_time):
    ...
    try:
        ...
        for i in range(10):
            try:
                day = new_head[i][2] + date[new_head[i][0]] + new_head[i][1]
                if '@' in new_head[i][5]:
                    flag = 0
                    loss = new_head[i][5].replace('@', '')
                else:
                    flag = 1
                    loss = new_head[i][5]
                win = new_head[i][3]
                l = [day, win, loss, flag, int(new_head[i][4]), int(new_head[i][6])]
            except:
                l = [None, None, None, None, None, None]

            head_to_head.append(l)
    except:
        head_to_head = []
        for i in range(10):
...
def get_2(year, data_time):
    url = 'http://www.baseball-reference.com/previews/' + year + '/' + data_time + '.shtml'
    data = pd.read_html(url)
    x = data[0]
    x.columns = range(x.shape[1])
    x = x[x[0].notnull()]
    x = x.loc[list(map(lambda x: len(x) > 15, x[0]))]
    x = x.loc[:, 0]
...
def GetData(year, data_time):
    try:
        return get_1(year, data_time)
    except:
        return get_2(year, data_time)
...
if __name__ == '__main__':
    ...
    for year in years:
        ...
        for i in range(length):
            if os.path.exists( path + 'data-' + year + data_times[i][:3] + '.json'):
                continue
            ...
            print('%s %s %.2f%%' % (year, team_name, float(i)/length*100))
            try:
